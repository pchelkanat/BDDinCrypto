...
import PBDD.BDD_ite as ite

from PBDD import dot_bdd as dbdd
from PBL import PyBool_public_interface as pb
...
def bdd_create(string_bdd):
    ...
    lines = [s + "\n" for s in [s for s in string_bdd.splitlines() if s != ""]]
...
def reorder_ite_build(bdd, vars_to_consider=[], verbose=False):
    ...
    while False in list(checked_vars.values()):
